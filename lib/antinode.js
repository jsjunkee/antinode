...
exports.start = function(custom_settings, callback) {
    ...
    server = http.createServer(function(req,resp) {
        ...
        if (vhost['handle'] !== undefined) {
            ...
            if (path.match(/\.sjs$/)) {
                execute_sjs(settings.script_cache, path, req, resp, vhost);
            } else {
...
function execute_sjs(cache, path, req, resp, host) {
    if(!cache) {
        ...
        fs.readFile(path, 'utf8', function(err, script) {
            try {
                if(err) throw err;
                sjs_cache[path] = new Script(script); // very nice, but you have to restart the sever if you have got a new script.. *ugly*
